...
function validateFileDialogCall(args, method) {
    ...
    validateProperties(
        ...
        [{  name: FILEDIALOG_PARAM_MODE, type: TYPE_STRING, 
            allowedValues: [FILEDIALOG_PARAM_MODE_OPEN, FILEDIALOG_PARAM_MODE_SAVE], optional: false },
...
function createLoginRequest(properties, onAuthRequestCompleted) {
    return new AuthRequest(AUTH_REQUEST_LOGIN, properties, onAuthRequestCompleted);
}
...
AuthRequest.prototype = {
    ...
    _onAuthChanged: function () {
        var response = wl_app._session.tryGetResponse(this._properties.normalizedScope, this._requestTs);
        if (response != null) {
...
AuthSession.prototype = {
    ...
    tryGetResponse: function (scope, requestTs) {
        ...
        if (lastReqTs >= requestTs) {
            ...
            if (session && isScopeSatisfied(session[AK_SCOPE], scope)) {
                return sessionStatus;
            }
    ...
    FilePickerOperation.prototype = {
        ...
        _validateAuth: function () {
            ...
            if (wl_app._rpsAuth) {
                ...
                var isOpenPicker = (op._props[FILEDIALOG_PARAM_MODE] === FILEDIALOG_PARAM_MODE_OPEN),
                    scope = isOpenPicker ? SCOPE_SKYDRIVE : SCOPE_SKYDRIVE_UPDATE,
                    interfaceMethod = op._props[API_INTERFACE_METHOD],
...
wl_app[API_X_HTTP_LIVE_LIBRARY] = "Web/DEVICE_" + trimVersionBuildNumber("5.5.7531.3000");
